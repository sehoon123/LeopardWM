...
fn focused_placement(workspace: &Workspace, focus: usize) -> leopardwm_core_layout::WindowPlacement {
    workspace
                    ...
                    for focus in 0..3 {
                        for start_ratio in [0.0, 0.5, 1.0] {
                            ...
                            let centers = mode == CenteringMode::Center
                                || (mode == CenteringMode::OnOverflow
                                    && width > VIEWPORT.width);
                            if centers {
                                let focused_center =
                                    placement.rect.x as i64 + placement.rect.width as i64 / 2;
                                let viewport_center =
                                    VIEWPORT.x as i64 + VIEWPORT.width as i64 / 2;
